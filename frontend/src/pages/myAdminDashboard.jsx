...
// My custom admin dashboard - spent way too much time on this!
const MyAdminDashboard = () => {
  const { apiCall } = useAuth();
  const [dashboardData, setDashboardData] = useState(null);
  const [users, setUsers] = useState([]); // user data array
  const [stores, setStores] = useState([]); // store data array
  ...
  useEffect(() => {
    fetchDashboardData(); // load the main stats first
    if (activeTab === 'users') {
      fetchUsers(); // then get user data if needed
    } else if (activeTab === 'stores') {
      fetchStores(); // or store data
    }
    // console.log('Active tab changed to:', activeTab); // debug line I left in
  }, [activeTab]);
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      setDashboardData(data); // this took me forever to get working right
    } catch (error) {
      setError('Failed to fetch dashboard data');
      // console.log('Dashboard fetch error:', error); // keeping this for debugging
    } finally {
      setLoading(false);
    }
  };
...
export default MyAdminDashboard; // my personalized admin component